...
const heads = ["Date", "No Transaction", "Customer", "Total", "Status", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('transaction.sale.getdata'), {
        params: {
            page: page,
...
<template>
    ...
    <div class="flex items-center justify-between mb-4 sm:mb-6">
        <h1 class="text-2xl font-bold md:text-3xl text-slate-800">Sales</h1>
    </div>
    <div class="bg-white border rounded-sm shadow-lg border-slate-200" :class="isLoading && 'min-h-[40vh] sm:min-h-[50vh]'">
        <header class="items-center justify-between block px-4 py-6 sm:flex">
            <h2 class="font-semibold text-slate-800">
                All Sales <span class="text-slate-400 !font-medium ml">({{ pagination.total }})</span>
            </h2>
            <div class="flex justify-end mt-3 space-x-2 sm:mt-0 sm:justify-between">
                <!-- Filter -->
                <VFilter @search="searchHandle" />
                <VButton label="Add Sales" type="primary" @click="handleCreate" class="mt-auto" />
            </div>
        </header>
