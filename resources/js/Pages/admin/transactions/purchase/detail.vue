...
const heads = ["Product", "Desciption", "Quantity", "Price", "Total"]
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 pb-20"
        ...
        <div class="mt-5">
            <div class="grid md:grid-cols-4 grid-cols-1 gap-3 px-4">
                ...
                <div class="mb-2">
                    ...
                    <p class="text-sm">
                        {{ additional.data.pay_with_account.code }} - {{ additional.data.pay_with_account.name }}
                    </p>
                </div>
                <div class="mb-2">
                    <label class="font-bold text-base text-slate-800">Supplier</label>
                    <p class="text-sm">
                        {{ additional.data.supplier.name }}
                    </p>
                </div>
                <div class="mb-2">
        ...
        <div>
            ...
            <VDataTable :heads="heads" bordered :divide-y="false">
                <tr v-for="(data, index) in additional.data.purchase_details">
                    <td class=" px-4 whitespace-nowrap h-12">{{ data.product.name }}</td>
                    <td class=" px-4 whitespace-nowrap h-12">
                        {{ data.desciption ?? '-' }}
                    </td>
                    <td class=" px-4 whitespace-nowrap h-12"> {{ data.quantity }}</td>
                    <td class=" px-4 whitespace-nowrap h-12"> Rp. {{ parseInt(data.price_per_unit).toLocaleString('id-ID')
                    }}</td>
                    <td class=" px-4 whitespace-nowrap h-12"> Rp. {{ (parseInt(data.price_per_unit) *
                        parseInt(data.quantity)).toLocaleString('id-ID') }}</td>
                </tr>
                <tr class="h-20 border-t">
                    <td colspan="3" />
                    <td class="pl-4 h-12 font-semibold text-base" align="right">
                        Total :
                    </td>
                    <td class="pl-4 h-12 font-semibold text-base">
                        Rp. {{ parseInt(additional.data.purchase_details.reduce((a, b) => a +
                            (b.price_per_unit * b.quantity), 0)).toLocaleString('id-ID') }}
                    </td>
                </tr>
            </VDataTable>
