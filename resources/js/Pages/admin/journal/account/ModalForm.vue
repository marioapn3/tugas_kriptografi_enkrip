...
const handleSelectClassification = () => {
    // get axios
    axios.get(route('journals.accounts.generatecode', { account_category_id: form.value.account_category_id })
    ).then((res) => {
...
const create = async () => {
    isLoading.value = true
    axios.post(route('journals.accounts.create'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
        ...
        <template v-slot:content>
            <div class="grid grid-cols-2 gap-3">
                ...
                <div class="col-span-2">
                    <VSelect placeholder="Select Category" :required="true" v-model="form.account_category_id"
                        :options="additional.category_options" label="Select Category"
                        :errorMessage="formError.account_category_id" @update:modelValue="handleSelectClassification" />
                </div>
